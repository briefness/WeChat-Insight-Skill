...
- **按需生成 HTML**：完整 Markdown 文章生成后主动询问，用户确认后生成公众号兼容的同名 HTML 文件
...
为了避免文章停在“讲得都对，但读者不知道怎么做”，完整成稿还要求每个主体小节都有自然呈现的可执行单元，例如操作步骤、判断标准、检查清单、避坑链路、最小实验或可复用模板。整篇文章会按类型形成落地链路：教程讲清准备、步骤、验证和避坑；复盘讲清目标、取舍、实现、问题和验证；观察和推广也必须给出案例、边界和判断方法。

如果素材不足以支撑这些标准，Skill 不会硬凑成“可直接发布”稿，而会先补证据、追问最少量关键信息，或降级为明确标注的“方法论草稿”。输出状态分为三类：可直接发布、需要补素材、方法论草稿。

默认会优先自主取证：读取当前对话、项目文件、README、配置、测试命令、Git diff、已有产物和官方资料。只有缺失信息会影响核心事实或文章立场时，才向用户追问；能写成有边界的公众号草稿时，会先写草稿，把少量待确认项放在发布前检查里。

写产品、工具、平台或行业观察时，会把用户提供的亲历当作文章切口，而不是全部素材。Skill 会主动补齐产品定位、入口与模式、核心能力、扩展能力、运行环境、价格与限额、典型场景、不适用场景和最新状态，避免只围绕用户给出的几个点展开。

遇到多入口产品时，Skill 不只罗列功能，还会解释为什么拆分、拆分的优缺点、切换成本和选择标准。复盘、产品体验、行业观察和个人经验类文章结尾会保留作者自我思考，说明这次经历改变了什么判断、下次会怎么做，以及仍然有哪些边界。

完整公众号稿默认是图文稿：封面图必须生成，正文至少包含 1 张有信息价值的图片。只有用户明确要求纯文字、图片工具不可用或生成失败时，才降级为配图建议，并在发布前检查中说明原因。
...
完整文章保存为 Markdown 文件后，Skill 会询问是否需要转换为 HTML。用户确认后执行：

```bash
python3 /Users/lucas/Desktop/WeChat-Insight-Skill/tools/publish_to_wechat.py "<Markdown 文件绝对路径>" --html-only
```

HTML 默认生成在 Markdown 文件同目录，文件名保持一致，仅扩展名改为 `.html`。转换结果只保留可发布正文，不包含“使用假设”和“摘要”；Markdown 源文件保持不变。
